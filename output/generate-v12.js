...
// ==========================================================================
// SLIDE 06: Background & Literature - SIMPLIFIED: Ultra-Clean Visual Flowcards
// ==========================================================================
{
  const s = createSlide();
  ...
  const frameworks = [
    {
      name: "RCI",
      title: "Recursive Critique & Improvement",
      cite: "Kim et al. (NeurIPS 2023)",
      accent: C.primary,
      accentBg: C.primaryLight,
      nodes: ["Code (y0)", "🔍 Critique", "🛠️ Patched (y1)"],
      tag: "Recursive zero-shot error prompt & patch"
    },
    {
      name: "Reflexion",
      title: "Verbal Reinforcement Memory",
      cite: "Shinn et al. (NeurIPS 2023)",
      accent: C.green,
      accentBg: C.greenLight,
      nodes: ["Action Trial", "⚠️ Env Signal", "🧠 Memory (Mt)"],
      tag: "Episodic memory buffer across failed trials"
    },
    {
      name: "Self-Refine",
      title: "Multi-Aspect Feedback Loop",
      cite: "Madaan et al. (NeurIPS 2023)",
      accent: C.amberDark,
      accentBg: C.amberLight,
      nodes: ["Draft Output", "📋 Rubric", "✨ Refined"],
      tag: "Iterative feedback on style & constraints"
    },
    {
      name: "Debate",
      ...
      accent: C.slateDark,
      accentBg: C.cardBg,
      nodes: ["Agents A & B", "⚔️ Debate", "🤝 Consensus"],
      tag: "Multi-agent cross-examination & voting"
    }
  ];
  ...
  frameworks.forEach((f, i) => {
    ...
    // Top Tag / Name
    s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
      x: cx + 0.12, y: cy + 0.12, w: 0.9, h: 0.26,
    ...
    s.addText(f.name, {
      x: cx + 0.12, y: cy + 0.12, w: 0.9, h: 0.26,
      fontSize: 9.5, fontFace: BODY_FONT, color: f.accent, bold: true, align: "center", valign: "middle", margin: 0
    });

    // Title and Citation
    s.addText(f.title, {
      x: cx + 1.1, y: cy + 0.10, w: cardW - 1.2, h: 0.26,
      fontSize: 10.5, fontFace: TITLE_FONT, color: C.slateDark, bold: true, margin: 0
    });
    s.addText(f.cite, {
      x: cx + 1.1, y: cy + 0.34, w: cardW - 1.2, h: 0.20,
      fontSize: 8.5, fontFace: BODY_FONT, color: C.muted, italic: true, margin: 0
    });

    // Clean Visual Flowchart (3 Nodes + 2 Arrows)
    const nodeW = 1.15;
    const nodeH = 0.38;
    const nodeY = cy + 0.62;
    const gapN = 0.28;

    f.nodes.forEach((nText, ni) => {
      const nx = cx + 0.15 + ni * (nodeW + gapN);
      s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
        x: nx, y: nodeY, w: nodeW, h: nodeH,
        fill: { color: ni === 1 ? f.accentBg : C.cardBg },
        line: { color: ni === 1 ? f.accent : C.cardBorder, width: 0.8 },
        rectRadius: 0.05
      });
      s.addText(nText, {
        x: nx + 0.02, y: nodeY, w: nodeW - 0.04, h: nodeH,
        fontSize: 8, fontFace: BODY_FONT, color: ni === 1 ? f.accent : C.slateDark,
        bold: ni === 1, align: "center", valign: "middle", margin: 0
      });

      if (ni < 2) {
        addArrow(s, nx + nodeW + 0.04, nodeY + 0.10, 0.20, 0.16, f.accent);
      }
    });

    // Single Punchy Summary Pill (Minimal text!)
    s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
      x: cx + 0.15, y: cy + 1.14, w: cardW - 0.3, h: 0.34,
      fill: { color: C.cardBg },
      line: { color: C.cardBorder, width: 0.5 },
      rectRadius: 0.04
    });
    s.addText(f.tag, {
      x: cx + 0.20, y: cy + 1.14, w: cardW - 0.4, h: 0.34,
  ...
  // Bottom Takeaway Bar
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
    x: 0.6, y: 4.88, w: 8.8, h: 0.28,
    fill: { color: C.primaryLight },
    line: { color: C.primary, width: 0.6 },
    rectRadius: 0.04
  });
  s.addText("⚠️ Reported gains (+7% to +15%) across these frameworks were driven by hidden evaluation confounders.", {
    x: 0.7, y: 4.88, w: 8.6, h: 0.28,
...
// ==========================================================================
// SLIDE 07: Background & Literature - SIMPLIFIED: Pure Visual Flow Architecture
// ==========================================================================
{
  const s = createSlide();
  addSlideHeading(s, "2. Background & Related Work", "Intrinsic vs. External Feedback", "The crucial distinction between closed-loop hallucination and grounded verification");
  ...
  // -------------------------------------------------------------
  // LEFT COLUMN: INTRINSIC (CLOSED LOOP)
  // -------------------------------------------------------------
  const lx = 0.6;
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
  ...
  // Header Badge Left
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
    x: lx + 0.2, y: startY + 0.15, w: 3.85, h: 0.34,
  ...
  s.addText("INTRINSIC (Closed-Loop / Paper Focus)", {
    x: lx + 0.2, y: startY + 0.15, w: 3.85, h: 0.34,
    fontSize: 10, fontFace: BODY_FONT, color: C.white, bold: true, align: "center", valign: "middle", margin: 0
  });

  // Step 1
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
    x: lx + 0.25, y: startY + 0.60, w: 3.75, h: 0.38,
    fill: { color: C.white },
    line: { color: C.crimson, width: 0.7 },
    rectRadius: 0.05
  });
  s.addText("1. Draft Generation (y0)  •  Initial Chain of Thought", {
    x: lx + 0.35, y: startY + 0.60, w: 3.55, h: 0.38,
    fontSize: 9, fontFace: TITLE_FONT, color: C.slateDark, bold: true, valign: "middle", margin: 0
  });

  // Down Arrow 1
  s.addShape(pres.shapes.DOWN_ARROW, { x: lx + 2.0, y: startY + 1.04, w: 0.24, h: 0.14, fill: { color: C.crimson }, line: { color: C.crimson } });

  // Step 2
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
    x: lx + 0.25, y: startY + 1.22, w: 3.75, h: 0.48,
    fill: { color: C.white },
    line: { color: C.crimson, width: 0.7 },
    rectRadius: 0.05
  });
  s.addText('2. Intrinsic Critique  •  "Find flaws"', {
    x: lx + 0.35, y: startY + 1.24, w: 3.55, h: 0.22,
    fontSize: 9, fontFace: TITLE_FONT, color: C.crimson, bold: true, margin: 0
  });
  s.addText("Same frozen weights (θ)  •  Zero external ground truth", {
    x: lx + 0.35, y: startY + 1.46, w: 3.55, h: 0.20,
    fontSize: 8, fontFace: BODY_FONT, color: C.muted, margin: 0
  });

  // Down Arrow 2
  s.addShape(pres.shapes.DOWN_ARROW, { x: lx + 2.0, y: startY + 1.76, w: 0.24, h: 0.14, fill: { color: C.crimson }, line: { color: C.crimson } });

  // Step 3
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
    x: lx + 0.25, y: startY + 1.94, w: 3.75, h: 0.38,
    fill: { color: C.white },
    line: { color: C.crimson, width: 0.7 },
    rectRadius: 0.05
  });
  s.addText("3. Blind Revision (y1)  •  Unverified modification", {
    x: lx + 0.35, y: startY + 1.94, w: 3.55, h: 0.38,
    fontSize: 9, fontFace: TITLE_FONT, color: C.slateDark, bold: true, valign: "middle", margin: 0
  });

  // Result Badge Left
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
    x: lx + 0.25, y: startY + 2.45, w: 3.75, h: 0.75,
    fill: { color: C.white },
    line: { color: C.crimson, width: 1.2 },
    rectRadius: 0.06
  });
  s.addText("❌ OUTCOME: ACCURACY DROPS", {
    x: lx + 0.35, y: startY + 2.50, w: 3.55, h: 0.24,
    fontSize: 10.5, fontFace: TITLE_FONT, color: C.crimson, bold: true, align: "center", margin: 0
  });
  s.addText("• Performance declines (-1.3% to -27.5%)\n• Correct answers flip to incorrect (Net negative drift)", {
    x: lx + 0.35, y: startY + 2.76, w: 3.55, h: 0.40,
    fontSize: 8.5, fontFace: BODY_FONT, color: C.slateDark, align: "center", margin: 0, lineSpacingMultiple: 1.15
  });

  // -------------------------------------------------------------
  // RIGHT COLUMN: EXTERNAL (OPEN LOOP / GROUNDED)
  // -------------------------------------------------------------
  const rx = 5.15;
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
  ...
  // Header Badge Right
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
    x: rx + 0.2, y: startY + 0.15, w: 3.85, h: 0.34,
    fill: { color: C.green },
  ...
  // Step 1 Right
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
    x: rx + 0.25, y: startY + 0.60, w: 3.75, h: 0.38,
    fill: { color: C.white },
    line: { color: C.green, width: 0.7 },
    rectRadius: 0.05
  });
  s.addText("1. Program / Plan Generation  •  Code, SQL, formula", {
    x: rx + 0.35, y: startY + 0.60, w: 3.55, h: 0.38,
    fontSize: 9, fontFace: TITLE_FONT, color: C.slateDark, bold: true, valign: "middle", margin: 0
  });

  // Down Arrow 1 Right
  s.addShape(pres.shapes.DOWN_ARROW, { x: rx + 2.0, y: startY + 1.04, w: 0.24, h: 0.14, fill: { color: C.green }, line: { color: C.green } });

  // Step 2 Right
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
    x: rx + 0.25, y: startY + 1.22, w: 3.75, h: 0.48,
    fill: { color: C.white },
    line: { color: C.green, width: 0.7 },
    rectRadius: 0.05
  });
  s.addText("2. Deterministic Engine  •  REPL / Test Oracle", {
    x: rx + 0.35, y: startY + 1.24, w: 3.55, h: 0.22,
    fontSize: 9, fontFace: TITLE_FONT, color: C.green, bold: true, margin: 0
  });
  s.addText("Execution traceback  •  Exact error line diagnosis", {
    x: rx + 0.35, y: startY + 1.46, w: 3.55, h: 0.20,
    fontSize: 8, fontFace: BODY_FONT, color: C.muted, margin: 0
  });

  // Down Arrow 2 Right
  s.addShape(pres.shapes.DOWN_ARROW, { x: rx + 2.0, y: startY + 1.76, w: 0.24, h: 0.14, fill: { color: C.green }, line: { color: C.green } });

  // Step 3 Right
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
    x: rx + 0.25, y: startY + 1.94, w: 3.75, h: 0.38,
    fill: { color: C.white },
    line: { color: C.green, width: 0.7 },
    rectRadius: 0.05
  });
  s.addText("3. Conditioned Repair  •  Targeted bug fix", {
    x: rx + 0.35, y: startY + 1.94, w: 3.55, h: 0.38,
    fontSize: 9, fontFace: TITLE_FONT, color: C.slateDark, bold: true, valign: "middle", margin: 0
  });

  // Result Badge Right
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
    x: rx + 0.25, y: startY + 2.45, w: 3.75, h: 0.75,
    fill: { color: C.white },
    line: { color: C.green, width: 1.2 },
    rectRadius: 0.06
  });
  s.addText("✔️ OUTCOME: ACCURACY IMPROVES", {
    x: rx + 0.35, y: startY + 2.50, w: 3.55, h: 0.24,
    fontSize: 10.5, fontFace: TITLE_FONT, color: C.green, bold: true, align: "center", margin: 0
  });
  s.addText("• Genuine gains (+7% to +15%) via ground-truth signal\n• External tools provide objective error localization", {
    x: rx + 0.35, y: startY + 2.76, w: 3.55, h: 0.40,
    fontSize: 8.5, fontFace: BODY_FONT, color: C.slateDark, align: "center", margin: 0, lineSpacingMultiple: 1.15
  });

  // Bottom Principle Banner
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
    x: 0.6, y: 4.75, w: 8.8, h: 0.40,
...
// ==========================================================================
// SLIDE 08: Background & Literature - SIMPLIFIED: Visual Confounder Diagnostics
// ==========================================================================
{
  const s = createSlide();
  addSlideHeading(s, "2. Background & Related Work", "Flaws in Prior Evaluations", "Three fundamental confounders exposed by Huang et al. (Table 1)");

  const pillars = [
    {
      title: "Oracle Leakage",
      subtitle: "Affects: RCI, Reflexion",
      accent: C.primary,
      accentBg: C.primaryLight,
      icon: "⚠️",
      flaw: "Secret Ground-Truth Signal",
      points: [
        "Harness alerted model ONLY when wrong",
        "Protected correct answers from corruption",
        "Artificially boosted scores (+7% to +15%)",
        "Reality: No oracle exists in deployment"
      ]
    },
    {
      title: "Unfair Compute Parity",
      subtitle: "Affects: Multi-Agent Debate",
      accent: C.amberDark,
      accentBg: C.amberLight,
      icon: "⚖️",
      flaw: "Compute Budget Asymmetry",
      points: [
        "Debate consumed N x M model queries",
        "Compared against single-shot (N = 1) baseline",
        "Gains vanish vs Self-Consistency at equal cost",
        "Reality: Simple majority voting beats debate"
      ]
    },
    {
      title: "Prompt Design Distortion",
      subtitle: "Affects: Self-Refine",
      accent: C.slateDark,
      accentBg: C.cardBg,
      icon: "📝",
      flaw: "Withheld Task Constraints",
      points: [
        "Initial prompt omitted critical task rules",
        "Feedback prompt injected missing instructions",
        "Model merely followed delayed rules",
        "Reality: Complete prompt up-front wins"
      ]
    }
  ];
  ...
  pillars.forEach((p, i) => {
    const px = startX + i * (pillW + gapP);

    s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
      x: px, y: startY, w: pillW, h: pillH,
      fill: { color: C.white },
    ...
    // Top Header
    s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
      x: px + 0.12, y: startY + 0.12, w: pillW - 0.24, h: 0.58,
      fill: { color: p.accentBg },
      line: { color: p.accent, width: 0.6 },
      rectRadius: 0.05
    });
    s.addText(p.title, {
      x: px + 0.16, y: startY + 0.15, w: pillW - 0.32, h: 0.26,
      fontSize: 12, fontFace: TITLE_FONT, color: p.accent, bold: true, align: "center", margin: 0
    });
    s.addText(p.subtitle, {
      x: px + 0.16, y: startY + 0.42, w: pillW - 0.32, h: 0.22,
      fontSize: 8.5, fontFace: BODY_FONT, color: C.muted, italic: true, align: "center", margin: 0
    });

    // Flaw Badge
    s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
      x: px + 0.15, y: startY + 0.80, w: pillW - 0.3, h: 0.28,
      fill: { color: C.white },
      line: { color: p.accent, width: 0.8 },
      rectRadius: 0.04
    });
    s.addText(`${p.icon}  ${p.flaw}`, {
      x: px + 0.15, y: startY + 0.80, w: pillW - 0.3, h: 0.28,
      fontSize: 8.5, fontFace: BODY_FONT, color: p.accent, bold: true, align: "center", valign: "middle", margin: 0
    });

    // Clean Bullet Points Box
    s.addShape(pres.shapes.ROUNDED_RECTANGLE, {
      x: px + 0.12, y: startY + 1.18, w: pillW - 0.24, h: 2.02,
      fill: { color: C.cardBg },
      line: { color: C.cardBorder, width: 0.5 },
      rectRadius: 0.05
    });
    ...
    p.points.forEach((pt, pti) => {
      const ptY = startY + 1.28 + pti * 0.46;
      s.addText(`• ${pt}`, {
        x: px + 0.20, y: ptY, w: pillW - 0.40, h: 0.42,
        fontSize: 8.5, fontFace: BODY_FONT, color: pti === 3 ? p.accent : C.slateDark,
        bold: pti === 3, margin: 0, lineSpacingMultiple: 1.1
      });
    });
  });
...
const outPath = "output/LLM_Self_Correction_ICLR2024_Group_Presentation-v12.pptx";
pres.writeFile({ fileName: outPath })
  .then(() => console.log(`Presentation v12 generated successfully at: ${outPath}`))
